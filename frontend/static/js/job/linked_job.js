    ...
    // Fixed jobId function to use correct field names
    function jobId(job) { return job.id || `${job.title || ''}__${job.companyName || ''}`; }
    ...
    // ─────────────────────────────────────────
    // Build one job card - FIXED FIELD NAMES
    // ─────────────────────────────────────────
    function createJobItem(job) {
        ...
        // Use companyName instead of company_name
        const initials = (job.companyName || '?')
            .split(' ').slice(0, 2).map(w => w[0] || '').join('').toUpperCase() || '?';

        // Tags - using correct field names
        const tags = [];
        if (job.contractType) tags.push({ label: job.contractType, cls: job.contractType === 'Full-time' ? 'tag-ft' : 'tag-contract' });
        if (job.experienceLevel && job.experienceLevel !== 'Not Applicable') tags.push({ label: job.experienceLevel, cls: '' });
        if (job.location && /remote/i.test(job.location)) tags.push({ label: 'Remote', cls: 'tag-remote' });
        if (job.salary) tags.push({ label: job.salary, cls: 'tag-salary' });
        const tagsHTML = tags.map(t => `<span class="job-tag ${t.cls}">${esc(t.label)}</span>`).join('');

        // Applicants - using applicationsCount
        let appBadge = '';
        if (job.applicationsCount) {
            const early = /first/i.test(job.applicationsCount);
            appBadge = `<span class="applicants-badge ${early ? 'early' : 'active'}"><i class="bi bi-people-fill"></i> ${esc(String(job.applicationsCount))}</span>`;
        }
        ...
        // Using correct field names: jobUrl, companyUrl, postedTime, title
        const viewHref = job.jobUrl || '#';
        const compHref = job.companyUrl || '';
        const viewDis  = !job.jobUrl ? 'disabled' : '';
        const posted   = job.postedTime || job.publishedAt || 'N/A';
            ...
            <div class="job-header-row">
                ...
                <div class="job-info">
                    <div class="job-title-row">
                        <h3 class="job-title">${esc(job.title || 'N/A')}</h3>
                    ...
                    <div class="company-name"><i class="bi bi-building"></i> ${esc(job.companyName || 'N/A')}</div>
                    ${tagsHTML ? `<div class="tag-list">${tagsHTML}</div>` : ''}
                    <div class="job-meta">
                        <div class="job-meta-item"><i class="bi bi-geo-alt-fill"></i><span>${esc(job.location || 'Not specified')}</span></div>
                        <div class="job-meta-item"><i class="bi bi-clock-fill"></i><span>${esc(String(posted))}</span></div>
                        ${job.workType   ? `<div class="job-meta-item"><i class="bi bi-briefcase-fill"></i><span>${esc(job.workType)}</span></div>` : ''}
                        ${job.sector ? `<div class="job-meta-item"><i class="bi bi-building-fill"></i><span>${esc(job.sector)}</span></div>` : ''}
                    </div>
                </div>
            </div>
    ...
    function toggleSave(job, btn, itemEl) {
        ...
        if (wasSaved) {
            ...
            toast('info', `Removed "${esc(job.title)}" from saved`);
        } else {
            saveJob(job);
            ...
            animateBurst(btn);
            toast('success', `Saved "${esc(job.title)}"`, '❤️ Saved');
        }
        updateSavedBadges();
    }
